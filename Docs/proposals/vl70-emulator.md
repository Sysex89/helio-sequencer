...
3. **Closed-loop tuner.** Reed and bow junctions add phase that depends on Pressure and Embouchure, so the played pitch drifts by several cents as the controllers move. Analytic compensation cannot capture this. The voice therefore measures the period it is actually producing and nudges the loop length toward the target. The measurement correlates the bore signal with fractionally delayed copies of itself at the target period and one sample either side, and locates the peak by parabolic interpolation. Once the target sits on the peak the three points straddle it symmetrically, so the estimate is exact whatever the waveform looks like, which zero-crossing or peak-picking detectors are not on the pulse-like waveforms of bright reeds. The tuner waits for the note to settle (a fixed time and a number of periods, since low notes speak slowly), is disabled while a glide is in progress, moves most of the way each step, and never moves the loop by more than a few percent. In phase 1 this replaced the planned offline calibration tables: with the tuner in place the residual error is under a quarter of a cent on most presets, and the tables are no longer needed.
...
- **Pitch accuracy:** for each driver and resonator type, and for 12-EDO, 19-EDO and 31-EDO from the built-in temperaments, render half a second of a sustained note at five keys spread over the playable range with fixed pressure and embouchure, and measure the fundamental over the last 100 ms by autocorrelation. Assert within 3 cents of `Temperament::getNoteInHertz()`. This is the test that justifies "microtonal out of the box". Scala files are not tested: Helio loads Scala keyboard mappings, not Scala tunings, so built-in temperaments are the only tuning source. The same check runs for every factory preset.
- **Every preset sustains across the range:** each reed setting only oscillates within a band of mouth pressures, below which it does not speak and above which it chokes or period-doubles. Presets map velocity and breath into that band, and the test plays every preset at four keys and three velocities and asserts the note is still sounding after half a second. The bands were found by sweeping the model offline.
